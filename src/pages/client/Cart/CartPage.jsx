...
const CartPage = () => {
  ...
  const handleUpdateQuantity = async (id, newQuantity) => {
    ...
    try {
      const actionResult = await dispatch(updateCartItemQuantity({ idCartItem: id, quantity: newQuantity }));
      if (updateCartItemQuantity.rejected.match(actionResult)) {
        messageApi.error(actionResult.payload || "Failed to update quantity");
      }
    } catch (error) {
                    ...
                    sortedItems.map((item) => {
                      ...
                      return (
                        <div key={item.idCartItem} style={{
                          ...
                          {/* CartItem Component */}
                          <div style={{ flex: 1, filter: isInactive ? 'grayscale(100%)' : 'none' }}>
                            <CartItem
                              item={{
                                id: item.idCartItem,
                                name: item.nameProduct,
